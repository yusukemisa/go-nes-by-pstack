...
1. `$2007` の読みで VRAM アドレスを進める。`internal/ppu/ppu.go` の `CPURead`、`case PPUDATA` で `Increment` がコメントアウトされている。書き込み側の `CPUWrite` は進む。読みの更新を戻す。
2. 命令の実行を表に寄せるか、空の表を消す。`setupInstructionTable` は 256 個すべてを名前 `NOP`、`Operate` が nil で埋める。実際のサイクルは `baseCycles`。設計の本命は名前、バイト数、基本サイクル、実行を持つ `[256]Opcode`。表を実行経路にするなら nestest が赤くなった時点で止める。表を使わないなら `lookup` と `Disassemble` を消す。
3. ページクロスの加算を一箇所にする。`willCrossPage` と、`enhanced_cpu.go` 内の `totalCycles++` が両方ある。LDA abs,Y などは実行中の加算だけが効く。`willCrossPage` を真にすると二重になる。
